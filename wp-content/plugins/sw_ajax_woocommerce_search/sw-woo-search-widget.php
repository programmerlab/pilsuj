...
class sw_woo_search_widget extends WP_Widget {
	...
	function __construct(){
		...
		add_filter( 'posts_where', array( $this, 'custom_product_search' ) );
	...
	*/
	...
	function custom_product_search( $where ){
		if( is_search() ){
			$search_sku  = ( isset($_GET['search_sku']) && $_GET['search_sku'] ) ? $_GET['search_sku'] : 0;
			$filter_name = ( isset($_GET['s']) && $_GET['s'] ) ? $_GET['s'] : '';
			global $wpdb;
			if( $search_sku ){
				$where = " AND ((({$wpdb->posts}.post_title LIKE '%{$filter_name}%') OR ({$wpdb->posts}.post_excerpt LIKE '%{$filter_name}%') OR ({$wpdb->posts}.post_content LIKE '%{$filter_name}%'))  OR 
				( ( {$wpdb->postmeta}.meta_key = '_sku' AND {$wpdb->postmeta}.meta_value LIKE '%{$filter_name}%' ) ) ) 
