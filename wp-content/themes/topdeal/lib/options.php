...
if ( is_admin()){
	add_filter('topdeal-opts-rights', 'add_field_rights', 10, 2);
}

function topdeal_options(){
	global $topdeal_options;
	return $topdeal_options;
}

$add_query_vars = array();
function topdeal_query_vars( $qvars ){
...
if (!is_admin()){
	add_filter('query_vars', 'topdeal_query_vars');
	add_action('parse_request', 'topdeal_parse_request');
}
?>